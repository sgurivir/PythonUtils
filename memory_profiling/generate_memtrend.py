...
class MemgraphProcessor:
    ...
    @staticmethod
    def footprint_for(path_to_memgraph, daemon="locationd"):
        # The line producted by footprint command is like below. Extract footprint from the line
        # daemon [76] (memgraph): 64-bit\tFootprint: 71 MB (16384 bytes per page)\n'
    ...
    @staticmethod
    def footprint_from_memgraphs(paths_to_memgraphs, daemon="locationd"):
        """
        ...
        for memgraph in paths_to_memgraphs:
            timestamp = MemgraphProcessor.timestamp_for(memgraph)
            footprint = MemgraphProcessor.footprint_for(memgraph, daemon)
...
class MemoryTools:
    ...
    @staticmethod
    def find_memgraphs_with_inactive_limit_breach(paths_to_memgraphs, out_csv):
        """
        ...
        with open(out_csv, "w") as f_:
            ...
            for memgraph in paths_to_memgraphs:
                os_transaction_count = 0

                with subprocess.Popen(["heap", "-q", "-s", "--guessNonObjects", "--addresses='.*_os_*transaction'", memgraph],
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE) as p_1:
                ...
                if os_transaction_count > 0:
                    footprint = MemgraphProcessor.footprint_for(memgraph)
                    timestamp = MemgraphProcessor.timestamp_for(memgraph)
                    f_.write(memgraph, timestamp, footprint)
                else:
                    print(f"Num OS transactions: {os_transaction_count}")
...
if __name__ == '__main__':
    ...
    MemoryTools.find_memgraphs_with_inactive_limit_breach(paths_to_memgraphs=memgraphs,
    ...
    timestamps_and_footprints = MemgraphProcessor.footprint_from_memgraphs(paths_to_memgraphs=memgraphs,
                                                                           daemon=args.daemon_name)
